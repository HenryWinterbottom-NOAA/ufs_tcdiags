...
### TODO FIX TABLE OF FIGURES BELOW



Note that regions relative to
the respective TCs have not been excluded from the
calculations.
		
Tropical cyclone sea-level pressure (hPa) (top) and 10-meter wind
speed (mps) (bottom) intensity (bottom). The areas of missing data are
locations where the Bister and Emanuel (2002) solution does not
converge.

The above results are produced using the Jupyter notebook
``jupyter/notebooks/tcdiags.be2002_pi.ipynb``.
